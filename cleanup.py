...
# copied from http://www.pybloggers.com/2016/02/how-to-always-execute-exit-functions-in-python/


if os.name != 'posix':
    raise ImportError("POSIX only")
_registered_exit_functions = set()
_executed_exit_functions = set()
...
def register_exit_fun(fun, signals=_exit_signals):
    ...
    """
    def fun_wrapper():
        if fun not in _executed_exit_functions:
            try:
                fun()
            finally:
                _executed_exit_functions.add(fun)
    ...
    for sig in signals:
        ...
        if old_handler not in (signal.SIG_DFL, signal.SIG_IGN):
            ...
            # new fun).
            if old_handler not in _registered_exit_functions:
                atexit.register(old_handler)
                _registered_exit_functions.add(old_handler)

    # This further registration will be executed in case of clean
    # interpreter exit (no signals received).
    if fun not in _registered_exit_functions or not signals:
        atexit.register(fun_wrapper)
        _registered_exit_functions.add(fun)